  ...
  return (
    <div className={`cart-panel ${isOpen ? 'open' : ''}`}>
      <div className="cart-header">
        <h2>Your cart</h2>
        <a onClick={handleStartShopping} className="view-cart">View cart</a>
        ...
        {cartItems.length === 0 ? (
          ...
          <>
            <ul className="cart-items">
              {cartItems.map(item => (
                <li key={item.id} className="cart-item">
                  <img src={item.images[0]} alt={item.name} className="cart-item-thumbnail" />
                  <div className="cart-item-details">
                    ...
                    <div className="price-section">
                      ...
                      <span className="old-price">KSh{(item.oldPrice || item.price * 1.6).toLocaleString()}</span>
                    </div>
              ...
              <div className="suggested-list">
                {suggestedProducts.map(p => (
                  <div key={p.id} className="suggested-item">
                    ...
                    <button onClick={() => addToCart({ ...p, quantity: 1 })}>Add</button>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="note-shipping-section">
              ...
              <span>Taxes, discounts and <a href="#">shipping</a> calculated at checkout.</span>
            </div>

            <button className="order-now-btn">🛍️ Order Now</button>
            <button className="checkout-btn">🧾 Checkout - KSh{totalAmount.toLocaleString()}</button>
          </>
        )}
      </div>
    </div>
